// ============================================================
// CAT FUNTS TBL v3 â Main App Component
// ============================================================
...
export default function App() {
  ...
  const [adminCurrency, setAdminCurrency] = useState("Â£");
    ...
    setTimeout(() => {
      ...
      if (shareNow && lb.length > 1) {
        const comp = getComparisonCommentary({
          ...
          rank: rank + 1,
          total: lb.length,
          pct: calcPctLoss(me.startWeight, weightKg).toFixed(1),
          leader: leader?.nickname || leader?.name || "",
          leadPct: leader?.pctLoss?.toFixed(1) || "0",
          diff: Math.abs((leader?.pctLoss || 0) - calcPctLoss(me.startWeight, weightKg)).toFixed(1),
          aheadName: ahead?.nickname || ahead?.name || "",
          behindName: behind?.nickname || behind?.name || "",
          gap: ahead ? Math.abs(ahead.pctLoss - calcPctLoss(me.startWeight, weightKg)).toFixed(1) : "0",
          oldRank: myRank + 1,
          newRank: rank + 1,
          overtakenName: "",
          overtakerName: "",
  ...
  useEffect(() => {
    if (view === "admin" && competition) {
      ...
      setAdminCurrency(competition.currency || "Â£");
  ...
  const genPersonalMsg = () => {
    ...
    return `âï¸ FAT CUNTS âï¸\n${banter ? banter + "\n" : ""}\n${me.nickname}: ${Math.abs(pct).toFixed(1)}% total ${dir} ${emoji}\nð ${myRank + 1}/${players.length} on the board\nâ±ï¸ ${daysLeft} days to go`;
  };

  const genLeaderboardMsg = () => {
    if (leaderboard.length === 0) return "";
    let msg = `ð FAT CUNTS â WEEK ${currentWeek} ð\n\n`;
    leaderboard.forEach((p, i) => {
      const medal = i === 0 ? "ð¥" : i === 1 ? "ð¥" : i === 2 ? "ð¥" : `${i + 1}.`;
      const dir = p.pctLoss > 0 ? "â" : p.pctLoss < 0 ? "â" : "â";
      msg += `${medal} ${p.nickname}: ${p.pctLoss !== 0 ? `${Math.abs(p.pctLoss).toFixed(1)}% ${dir}` : "no change"}\n`;
    });
    if (currentSprint) {
      msg += `\nSprint ${currentSprint.number}: Week ${currentSprint.startWeek}â${currentSprint.endWeek}`;
    }
    msg += `\n${daysLeft} days left. ${competition?.currency || "Â£"}${totalPot} in the pot. ð°`;
    return msg;
  };
  ...
  const genSprintMsg = (sprint, results) => {
    ...
    if (!winner) return "";
    return `ð¨ FAT CUNTS â SPRINT ${sprint.number} RESULT ð¨\n\nð ${winner.nickname} takes it! ${winner.pctChange.toFixed(1)}% lost.\nPockets Â£${Math.round(totalPot * sprint.prizePercent / 100)}.\n\nSprint ${sprint.number + 1} starts now. All to play for.`;
  };
  ...
  const myChartData = useMemo(() => {
    ...
    return myWi.map((w) => ({
      date: fmtDate(w.date),
      weight: parseFloat(fromKg(w.weight, myUnit).toFixed(1)),
      shared: w.shared,
      id: w.id,
    }));
  }, [weighins, me, myUnit]);
      ...
      players.forEach((p) => {
        ...
        if (pWi.length > 0) {
          ...
          point[p.nickname || p.name] = parseFloat(
            calcPctLoss(p.startWeight, latest.weight).toFixed(1)
          );
        }
      });
      return point;
      ...
      <div style={S.app}>
        ...
        <div style={S.container}>
          ...
          <div style={S.card}>
            ...
            <input style={{ ...S.input, marginTop: 12 }} placeholder="Buy-in (Â£ per person)" type="number" value={formBuyIn} onChange={(e) => setFormBuyIn(e.target.value)} />
          </div>
    ...
    <div style={S.app}>
      ...
      <div style={S.container}>
        ...
        <div style={S.subtitle}>
          {competition?.code} â¢ {daysLeft} days left
          {myData?.streak >= 2 && (
        ...
        {view === "dashboard" && (
          <>
            ...
            <div style={{ ...S.card, display: "flex", gap: 8 }}>
              ...
              <div style={S.statBox}>
                <div style={S.statVal}>{competition?.currency || "Â£"}{totalPot}</div>
            ...
            {currentSprint && (
              <div style={{ ...S.card, padding: "12px 16px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                ...
                <span style={{ fontSize: 11, color: COLOURS.dim }}>
                  Weeks {currentSprint.startWeek}â{currentSprint.endWeek} â¢ {Math.max(0, currentSprint.endWeek - currentWeek + 1)} wks left
                </span>
              </div>
            )}
                ...
                return leaderboard.map((p) => {
                  ...
                  return (
                    <div key={p.id} style={{ ...lbRowStyle(me && p.id === me.id, rank ? rank - 1 : 99), ...(p.isGhost ? { opacity: 0.4 } : {}) }}>
                      <div style={lbRankStyle(rank ? rank - 1 : 99)}>{rank || "â"}</div>
                      <div style={{ flex: 1 }}>
                        ...
                        <div style={{ fontSize: 11, color: COLOURS.faint, marginTop: 2 }}>
                          {p.isGhost
                            ...
                            : `${p.totalSharedWeighins} weigh-in${p.totalSharedWeighins !== 1 ? "s" : ""} â¢ ${p.lastSharedDate ? fmtDate(p.lastSharedDate) : "no updates"}`}
                        </div>
                      </div>
                      <div style={p.isGhost ? { color: COLOURS.faint, fontSize: 15, fontWeight: 700 } : pctStyle(p.pctLoss)}>
                        {p.isGhost ? "â" : (p.pctLoss !== 0 ? `${Math.abs(p.pctLoss).toFixed(1)}%` : "â")}
                      </div>
                    </div>
                  );
                });
          ...
          <>
            <div style={S.card}>
              ...
              <input
                ...
                placeholder="Note (optional â e.g. post-curry, pre-run)"
                value={formNote}
                onChange={(e) => setFormNote(e.target.value)}
                maxLength={60}
              />
              <div style={{ fontSize: 12, color: COLOURS.faint, textAlign: "center", marginTop: 8 }}>
                Start: {myData ? fmtWeight(myData.startWeight, myUnit) : "â"} â¢ Last: {myData ? fmtWeight(myData.currentWeight, myUnit) : "â"} â¢ Change: {myData ? `${myData.pctLoss >= 0 ? "+" : ""}${myData.pctLoss.toFixed(1)}%` : "â"}
              </div>
            ...
            <button style={S.btnPrimary} onClick={() => handleWeighIn(true)}>
              Share it â I've got nothing to hide
            </button>
                ...
                return (
                  <div key={sd.sprint.number} style={S.row}>
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={{ fontSize: 11, color: COLOURS.faint, marginTop: 2 }}>
                        Wks {sd.sprint.startWeek}â{sd.sprint.endWeek} â¢ {sd.sprint.prizePercent}% of pot
                      </div>
                    </div>
                    <div style={{ textAlign: "right" }}>
                      {myResult?.eligible ? (
                        <span style={pctStyle(myResult.pctChange)}>
                          {myResult.pctChange > 0 ? "+" : ""}{myResult.pctChange.toFixed(1)}%
                        </span>
                      ) : (
                        <span style={{ fontSize: 12, color: COLOURS.dim }}>
                          {myResult ? `${myResult.weighInCount}/2 weigh-ins` : "â"}
                        </span>
                      )}
                    </div>
                  </div>
            ...
            <div style={S.card}>
              ...
              {(competition?.sprints || DEFAULT_SPRINTS).map((s) => (
                <div key={s.number} style={{ ...S.row, justifyContent: "space-between" }}>
                  <span style={{ fontWeight: 600 }}>Sprint {s.number} (Wks {s.startWeek}â{s.endWeek})</span>
                  <span style={{ color: COLOURS.gold, fontWeight: 700 }}>
                    {s.prizePercent}% â¢ {competition?.currency || "Â£"}{Math.round(totalPot * s.prizePercent / 100)}
                  </span>
                </div>
              ))}
              <div style={{ ...S.row, justifyContent: "space-between", borderBottom: "none" }}>
                ...
                <span style={{ color: COLOURS.gold, fontWeight: 700 }}>
                  {OVERALL_PRIZE_PERCENT}% â¢ {competition?.currency || "Â£"}{Math.round(totalPot * OVERALL_PRIZE_PERCENT / 100)}
                </span>
              </div>
            </div>
                ...
                {sd.results.map((r, i) => (
                  <div key={r.playerId} style={lbRowStyle(me && r.playerId === me.id, i)}>
                    ...
                    <div style={pctStyle(r.pctChange)}>
                      {r.eligible ? `${r.pctChange > 0 ? "+" : ""}${r.pctChange.toFixed(1)}%` : "â"}
                    </div>
                  </div>
                ))}
              </div>
              ...
              {leaderboard.map((p, i) => (
                <div key={p.id} style={S.row}>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ fontSize: 11, color: COLOURS.faint, marginTop: 2 }}>
                      {fmtWeight(p.startWeight, p.preferredUnit || "kg")} â {fmtWeight(p.currentWeight, p.preferredUnit || "kg")} â¢ {p.totalSharedWeighins} weigh-in{p.totalSharedWeighins !== 1 ? "s" : ""}
                    </div>
                  </div>
                  <div style={pctStyle(p.pctLoss)}>
                    {p.pctLoss !== 0 ? `${p.pctLoss > 0 ? "+" : ""}${p.pctLoss.toFixed(1)}%` : "â"}
                  </div>
                </div>
              ))}
            </div>
        ...
        {view === "pot" && (
          <>
            <div style={S.card}>
              <div style={S.statBox}>
                <div style={{ ...S.statVal, fontSize: 42 }}>{competition?.currency || "Â£"}{totalPot}</div>
                <div style={S.statLabel}>Total Pot</div>
                <div style={{ color: COLOURS.faint, fontSize: 12, marginTop: 8 }}>
                  {competition?.currency || "Â£"}{competition?.buyIn || 0} per person
                </div>
              </div>
            </div>
            ...
            <div style={S.card}>
              ...
              {(competition?.sprints || DEFAULT_SPRINTS).map((s) => (
                <div key={s.number} style={{ ...S.row, justifyContent: "space-between" }}>
                  ...
                  <span style={{ color: COLOURS.gold, fontWeight: 700 }}>
                    {competition?.currency || "Â£"}{Math.round(totalPot * s.prizePercent / 100)}
                  </span>
                </div>
              ))}
              <div style={{ ...S.row, justifyContent: "space-between", borderBottom: "none" }}>
                ...
                <span style={{ color: COLOURS.gold, fontWeight: 900 }}>
                  {competition?.currency || "Â£"}{Math.round(totalPot * OVERALL_PRIZE_PERCENT / 100)}
                </span>
              </div>
            </div>
            ...
            {leaderboard.length > 0 && leaderboard[0].pctLoss > 0 && (
              <div style={{ ...S.card, textAlign: "center" }}>
                ...
                <div style={{ fontSize: 14, color: COLOURS.muted, marginTop: 4 }}>
                  {leaderboard[0].pctLoss.toFixed(1)}% lost â pocketing {competition?.currency || "Â£"}{Math.round(totalPot * OVERALL_PRIZE_PERCENT / 100)} if it holds
                </div>
              </div>
            )}
            ...
            <div style={S.card}>
              ...
              {adminStartDate && adminEndDate && (
                <div style={{ fontSize: 11, color: COLOURS.dim, marginTop: 8, textAlign: "center" }}>
                  ...
                  {" Â· "}Sprints: {calculateSprintsFromDates(adminStartDate, adminEndDate).map((s) => `${s.startWeek}â${s.endWeek}`).join(", ")}
                </div>
              )}
            </div>
            <div style={S.card}>
