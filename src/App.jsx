...
export default function App() {
  ...
  // ---- LOAD COMPETITION FROM LOCAL STORAGE (comp ID) ----
  useEffect(() => {
    if (!authUser) return;
  ...
  const handleJoin = async () => {
    ...
    await addWeighin(comp.id, {
      playerId,
      weight: weightKg,
      shared: true,
      note: "Starting weight",
      weekNumber: getCompWeek(now, fromTimestamp(comp.startDate)),
  ...
  const handleWeighIn = async (shareNow) => {
    ...
    const isFirst = myWeighins.length <= 1;
  ...
  const genLeaderboardMsg = () => {
    ...
    leaderboard.forEach((p, i) => {
      ...
      const dir = p.pctLoss > 0 ? "↓" : p.pctLoss < 0 ? "↑" : "–";
    ...
    msg += `\n${daysLeft} days left. ${competition?.currency || "£"}${totalPot} in the pot. 💰`;
    return msg;
  };
        ...
        {view === "dashboard" && (
          <>
            ...
            <div style={{ ...S.card, display: "flex", gap: 8 }}>
              <div style={S.statBox}>
                <div style={S.statVal}>{players.length}</div>
            ...
            <div style={{ ...S.card, padding: 0, overflow: "hidden" }}>
              ...
              {leaderboard.map((p, i) => (
                <div key={p.id} style={lbRowStyle(me && p.id === me.id, i)}>
                  ...
                  <div style={pctStyle(p.pctLoss)}>
                    {p.pctLoss !== 0 ? `${Math.abs(p.pctLoss).toFixed(1)}%` : "–"}
                  </div>
                </div>
              ))}
            </div>
          ...
          <>
            <div style={S.card}>
              ...
              <div style={{ fontSize: 12, color: COLOURS.faint, textAlign: "center", marginTop: 8 }}>
                Start: {myData ? fmtWeight(myData.startWeight, myUnit) : "–"} • Last: {myData ? fmtWeight(myData.currentWeight, myUnit) : "–"} • Change: {myData ? `${myData.pctLoss >= 0 ? "+" : ""}${myData.pctLoss.toFixed(1)}%` : "–"}
              </div>
                ...
                {sd.results.map((r, i) => (
                  <div key={r.playerId} style={lbRowStyle(me && r.playerId === me.id, i)}>
                    ...
                    <div style={pctStyle(r.pctChange)}>
                      {r.eligible ? `${r.pctChange > 0 ? "+" : ""}${r.pctChange.toFixed(1)}%` : "–"}
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div style={S.card}>
              ...
              {leaderboard.map((p, i) => (
                <div key={p.id} style={S.row}>
                  ...
                  <div style={pctStyle(p.pctLoss)}>
                    {p.pctLoss !== 0 ? `${p.pctLoss > 0 ? "+" : ""}${p.pctLoss.toFixed(1)}%` : "–"}
                  </div>
                </div>
              ))}
            </div>
            ...
            {leaderboard.length > 0 && leaderboard[0].pctLoss > 0 && (
              <div style={{ ...S.card, textAlign: "center" }}>
                ...
                <div style={{ fontSize: 14, color: COLOURS.muted, marginTop: 4 }}>
                  {leaderboard[0].pctLoss.toFixed(1)}% lost – pocketing {competition?.currency || "£"}{Math.round(totalPot * OVERALL_PRIZE_PERCENT / 100)} if it holds
                </div>
              </div>
            )}
